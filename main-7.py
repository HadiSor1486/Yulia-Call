        ...
        while True:
            ...
            if mt == "stream_control":
                ...
                if action == "play":
                    ...
                    # Streamer's video element fired 'ended'. Try to
                    # advance to next; if none, stop.
                    if st["idx"] + 1 < len(st["playlist"]):
                        st["idx"] += 1
                        st["time"] = 0.0
                        st["playing"] = True
                        ...
                        # End of playlist → end the stream entirely.
...
/* When a stream is active, body gets the .streaming class. The seat
   panel above shrinks to a thin horizontal strip so the video can take
   the main space. Avatars become smaller, names dim, and the handle
   hides — players can still see who's in the call but the focus shifts
   to the shared video.
   v3.29: actually target the correct class names (.seat, .seat-av,
   .seat-av-wrap, .seat-name) — previous version had a typo (.seat-tile-*)
   so the rules matched nothing and the panel didn't shrink. */
body.streaming .seat-panel{max-height:74px;min-height:74px}
body.streaming .seat-grid-wrap{padding:8px 12px 4px}
body.streaming .seat-grid{gap:10px;align-items:center}
body.streaming .seat{gap:2px;width:56px}
body.streaming .seat-av-wrap{width:40px;height:40px}
body.streaming .seat-av{width:40px;height:40px;font-size:14px;border-width:2px}
body.streaming .seat-name{font-size:10px;letter-spacing:0}
body.streaming .seat-name-row{gap:3px}
body.streaming .seat-dot{width:5px;height:5px}
body.streaming .seat-handle{display:none}
body.streaming .seat-badge{font-size:8px;padding:1px 4px;border-radius:5px}
body.streaming .seat-mute{width:14px;height:14px}
body.streaming .seat-mute svg{width:8px;height:8px}

/* The stream panel itself */
.stream-panel{display:none;position:relative;z-index:7;flex-shrink:0;background:#000;border-bottom:1px solid rgba(255,255,255,0.06)}
body.streaming .stream-panel{display:block}
/* v3.30: FIXED 16:9 landscape frame, like Twitch/Discord watch-parties.
   The container always has the same shape so chat keeps its space below.
   Portrait videos appear centered with black bars on the sides;
   landscape videos fill the frame; wide cinema videos get thin bars top
   and bottom. Either way nothing gets cropped and the layout stays
   predictable. We use `width:100%` of whatever the chat-stack column is,
   plus `aspect-ratio:16/9` to set the height proportionally — no
   max-height clamp (which had caused the v3.27 cropping). */
.stream-video-wrap{position:relative;width:100%;aspect-ratio:16/9;background:#000;overflow:hidden}
.stream-video-wrap video{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#000;display:block}
...
.stream-controls{padding:8px 14px 10px;background:linear-gradient(180deg,#0a0a0a,#000);color:#fff}
.stream-title{font-size:12px;font-weight:600;opacity:0.85;margin-bottom:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;letter-spacing:0.1px;color:#ddd}
.stream-progress{position:relative;height:4px;background:rgba(255,255,255,0.1);border-radius:3px;cursor:pointer;touch-action:none;transition:height .12s}
body.streaming.is-streamer .stream-progress{cursor:pointer}
body.streaming.is-streamer .stream-progress:hover{height:6px}
/* v3.30: warm orange-amber progress fill, matches the play button. */
...
.stream-progress-handle{position:absolute;top:50%;transform:translate(-50%,-50%);width:12px;height:12px;border-radius:50%;background:#fff;left:0%;display:none;box-shadow:0 1px 4px rgba(0,0,0,0.6),0 0 0 2px rgba(255,138,43,0.4)}
body.streaming.is-streamer .stream-progress-handle{display:block}
.stream-time-row{display:flex;justify-content:space-between;font-size:11px;color:#999;margin-top:5px;font-variant-numeric:tabular-nums}
.stream-buttons{display:flex;align-items:center;justify-content:center;gap:14px;padding:8px 0 2px}
...
.stream-viewer-info{text-align:center;font-size:12px;color:#bbb;padding:6px 0 2px;font-weight:500}
...
.stream-btn{width:34px;height:34px;border-radius:50%;border:none;background:rgba(255,255,255,0.08);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;padding:0;transition:transform .1s,background .15s}
.stream-btn:hover{background:rgba(255,255,255,0.14)}
.stream-btn:active{transform:scale(0.9)}
.stream-btn:disabled{opacity:0.3;cursor:not-allowed}
.stream-btn:disabled:active{transform:none}
.stream-btn svg{width:16px;height:16px}
/* v3.30: warm orange play button (was blue). Smaller and friendlier. */
.stream-btn-play{width:44px;height:44px;background:#ff8a2b;box-shadow:0 3px 10px rgba(255,138,43,0.32)}
.stream-btn-play:hover{background:#ff9a45}
.stream-btn-play svg{width:20px;height:20px}
.stream-btn-stop{background:rgba(255,59,48,0.18);color:#ff5f5a}
.stream-btn-stop:hover{background:rgba(255,59,48,0.3)}
.stream-btn-stop svg{width:14px;height:14px}
...
<div class="stream-panel" id="streamPanel">
  ...
  <div class="stream-controls" id="streamControls">
    ...
    <!-- Progress bar — visible to all, draggable only for streamer -->
...
// Also: viewer "tap to unmute" — clicking the video itself unmutes for
// viewers (not streamer, who has the native shield off).
(function attachTapShield() {
  const sh = document.getElementById('streamTapShield');
  if (!sh) return;
  sh.addEventListener('click', () => {
    if (streamCli.videoMuted) streamUserUnmute();
  });
})();
