...
*/
.games-header-btn{width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:none;border:none;cursor:pointer;padding:0;font-size:22px;line-height:1}
.games-header-btn:active{transform:scale(0.9)}
...
.uno-chat-btn{width:36px;height:36px;border-radius:50%;border:none;background:rgba(255,255,255,0.1);color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;position:relative}
...
.uno-hand .uno-card{margin-left:-26px;transition:transform .2s,margin-left .2s}
.uno-hand .uno-card:first-child{margin-left:0}
.uno-hand .uno-card.playable{cursor:pointer}
.uno-hand .uno-card.playable:active{transform:translateY(-10px) scale(1.03)}
.uno-hand .uno-card.unplayable{opacity:0.55;filter:grayscale(0.35)}
...
<button class="games-header-btn" onclick="openGamesPicker()" title="Games" aria-label="Games">🎮</button>
...
<div class="games-picker-ovl" id="gamesPickerOvl" onclick="if(event.target===this)closeGamesPicker()">
  <div class="games-picker-box">
    ...
    <div class="games-picker-title">🎮 Games</div>
...
<div class="uno-winner" id="unoWinner">
  ...
  <div class="uno-winner-actions">
    <button class="uno-btn-primary" onclick="onWinnerClose()" style="padding:12px 22px">Done</button>
  </div>
</div>
...
function unoOnEvent(m) {
  ...
  if (m.kind === 'win') {
    ...
    document.getElementById('unoWinnerTitle').textContent =
      (unoCli.state && m.peer_id === MY_ID) ? 'You Won!' : 'Winner!';
    document.getElementById('unoWinnerName').textContent =
      t.replace(/^🏆\s*/, '').replace(/\s*wins.*$/, '');
...
function unoRenderPlay() {
  ...
  ds.innerHTML = '';
  if (s.top_card) {
    ds.appendChild(unoBuildCard(s.top_card, { size: 'md' }));
  }
  // Color dot
...
function toggleUnoChat() {
  ...
  if (unoCli.chatPanelOpen) {
    ...
    setTimeout(() => { try { document.getElementById('unoChatInput').focus(); } catch(e){} }, 250);
  } else {
