...
# v3.23: Uno game module — self-contained, only touched via handle_ws /
# on_peer_leave / on_room_cleanup. All state lives in uno.games.
import uno as uno_mod
# v3.24: Old-Maid-style Zombie game module. Same lifecycle pattern as uno.
import zombie as zomb_mod

# v4.0: LiveKit-backed voice (replaces all P2P mesh / TURN code).
...
# v3.22: per-room host avatar assignments. Maps room_id -> {peer_id: avatar_url}.
# Persists for the room's lifetime. Cleared on peer leave and on room cleanup.
host_assigned_avatars: Dict[str, Dict[str, str]] = {}

# v3.27: per-room streaming state.
#   room_id -> {
#     "streamer_pid": str,
#     "streamer_name": str,
#     "playlist": [{"id": "...", "url": "/streams/...", "title": "..."}, ...],
#     "idx": int,                       # current track index
#     "playing": bool,                  # play vs pause
#     "time": float,                    # currentTime at last_update
#     "last_update_at": float,          # server clock
#     "files": [absolute_path, ...],    # for cleanup
#   }
# Cleared when the streamer ends the stream, leaves the call, or the room
# is fully torn down. Files on disk are deleted on every cleanup path.
...
# v3.25: per-game background image folders.
...
# v3.27: in-room video streaming. The streamer uploads one or more video
...
# ── v3.26: game background file endpoints ────────────────────────────────
...
async def turn_endpoint():
    # v4.0: TURN/ICE no longer used by the client (LiveKit owns the transport).
    # We still expose this endpoint returning an empty list so any old cached
    # client that hits it doesn't 404 — they'll just get no ICE servers and
    # fall back to the LiveKit flow. Safe to delete once you're sure no one
    # is running the pre-v4.0 client.
    return JSONResponse({"iceServers": []})


# ─── v4.0: LIVEKIT VOICE ────────────────────────────────────────────────────
# We gate /livekit-token by the same invite-token table that protects
# /call/{room_id}, so only people with a valid call link can mint a
# LiveKit access token. This closure captures the `tokens` and `rooms`
# dicts from this module.
...
async def ws_endpoint(ws: WebSocket, room_id: str, t: str = Query(...)):
    ...
    # v3.22: sync any existing host-avatar assignments to the newcomer so
    ...
    # v3.27: sync any active stream state to the newcomer so they walk
    ...
    try:
        while True:
            ...
            # v3.22: explicit leave from the client. Faster than waiting for
            ...
            # ── v3.24: Zombie game dispatcher — same plumbing as Uno ──
            ...
            # v3.22: host assigns avatar to a peer. Only the host (admin) can
            ...
            # the new mapping and updates their UI to show the override.
                ...
                # v4.0: P2P mesh signaling is gone. Voice now flows through
        ...
        # v3.22: clean up any host-assigned avatar for this peer
        ...
        # v3.23: notify uno module that this peer left. uno will broadcast
        ...
        # v3.24: parallel Zombie hook — same _uno_send_after_leave callable
        ...
        # v3.27: if the leaver was the active streamer, kill the stream and
        ...
        if not room["peers"]:
            async def cleanup_later():
                ...
                if room_id in rooms and not rooms[room_id]["peers"]:
                    ...
                    # v3.22: also clean up host-assigned avatars for the room
                    if room_id in host_assigned_avatars:
                        del host_assigned_avatars[room_id]
                    # v3.23: drop any uno game state for the room
                    try:
                        uno_mod.on_room_cleanup(room_id)
                    except Exception as e:
                        print(f"[uno] on_room_cleanup error: {e}")
                    # v3.24: drop any zombie game state for the room
                    try:
                        zomb_mod.on_room_cleanup(room_id)
                    except Exception as e:
                        print(f"[zombie] on_room_cleanup error: {e}")
                    # v3.27: also tear down any stream + delete its files
                    if room_id in streams:
...
<!-- v4.0: LiveKit client SDK (UMD build for plain <script> consumption).
     Exposes the global `LivekitClient` with Room, RoomEvent, Track,
     RemoteParticipant, etc. No bundler needed. -->
...
<script>
// ════════════════════════════════════════════════════════════════════════════
// v4.0 — LIVEKIT SFU VOICE BOOTSTRAP
// ════════════════════════════════════════════════════════════════════════════
// What this block does:
//   • Connects to LiveKit when the server tells us our peer_id (`your_id`).
//   • Publishes our local mic track to LiveKit (replaces N-1 RTCPeerConnections).
//   • Attaches every remote participant's audio track to a hidden <audio>
//     element keyed by peer_id, so the rest of the existing UI code keeps
//     working (audio levels, speaking ring, mute states are all driven from
//     the same `audios[pid]` map and the same WS broadcasts).
//   • Neutralizes the old mesh-WebRTC functions (createOffer, handleOffer,
//     handleAnswer, handleIce, forceIceRestart, scheduleRetry) so any
//     stragglers from the old codebase or old cached clients just no-op.
//
// IMPORTANT: identities in LiveKit must match peer_ids the server hands out,
// so we delay the LiveKit connect until `your_id` arrives. The mic is
// acquired ONCE here and stored in `localStream` (same variable the rest of
// the code uses) so the mute toggle / level monitor / self-heal logic all
// keep working unchanged.
const LK = {
  room: null,                  // LivekitClient.Room instance
  connectedPid: null,          // our identity in LiveKit (== server peer_id)
  connecting: false,
  micPub: null,                // local mic publication
  remoteAudioByPid: {},        // pid -> <audio> element (mirror of `audios`)
  speakingTickerStartedAt: 0,
};

// Pull identifiers off LivekitClient now so a broken CDN load is obvious.
const _LKC = window.LivekitClient || null;
if (!_LKC) {
  console.error("[LK] LivekitClient SDK failed to load from CDN.");
}
...
async function lkConnect() {
  ...
  try {
    ...
    const room = new _LKC.Room({
      adaptiveStream: true,
      dynacast: true,
      // Voice-optimized publish defaults. 64kbps Opus is crystal-clear for
      // speech and uses ~30 KB/s upload per peer. LiveKit forwards once.
      publishDefaults: {
        audioPreset: { maxBitrate: 64000 },
        dtx: true,           // discontinuous transmission — saves bandwidth when silent
        red: true,           // packet-loss resilience (redundant audio encoding)
      },
...
// v3.22: host-assigned avatars by peer_id. When host assigns an avatar to a
...
// v3.22: pre-made avatars hosts can assign. Filled at page load from the
...
// v3.22: message click panel state (Copy / Reply popup over tapped messages)
...
// ════════════════════════════════════════════════════════════════════════════
// v3.22 — MESSAGE CLICK PANEL (Copy + Reply on others' text messages)
// ════════════════════════════════════════════════════════════════════════════
...
function connectWS() {
  ...
  ws.onmessage = async (ev) => {
    ...
    // v3.23: forward all uno_* messages to the Uno client module before the
    ...
    switch (m.type) {
      ...
      case 'your_id':
        ...
        // v4.0: now that we have our identity, connect to LiveKit.
      ...
      case 'chat':
        renderMsg(m);
        // v3.23: also mirror into the in-game Uno chat panel so players
        // can chat while playing (they're chatting in the same room).
        try { unoMirrorChat(m); } catch (e) {}
        // v3.24: same mirror for Zombie's in-game chat panel.
      ...
      case 'peer_left':
        ...
        // v3.22: also clear any host-assigned avatar override for this peer.
      ...
      // v3.22: host-assigned avatar update broadcast. Comes either from a
...
function _seatTile(opts) {
  ...
  // v3.22: avatar resolution priority:
...
function updPeers() {
  ...
  // v3.22: if I'm the host, attach click handlers to any seat avatar that's
...
function startConnectionTimer(pid, customTimeout) {
  ...
  pc._connTimer = setTimeout(() => {
    ...
    // v3.25: a "live" track that's actually receiving zero packets is a
  ...
  statsTimers[pid] = setInterval(async () => {
    ...
    try {
      ...
      if (!peerMuted && !inMuteGrace && dRecv === 0) {
        ...
        // v3.25: zombie-relay recovery. If the peer is on RELAY, has had
...
function cleanupRTC() {
  ...
  // v4.0: also tear down LiveKit so we don't leak the room connection.
  try { lkDisconnect(); } catch (e) {}
}

let _muteDebounceTimer = null;

// v3.26: returns the SVG <svg>...</svg> string for the mic icon in
...
function toggleMute() {
  ...
  // v4.0: keep LiveKit in sync so server-side speaker detection reflects
  ...
  // v3.26: sync ALL mute buttons (main call + in-game) in one pass.
  syncAllMuteBtns();
...
function attachMessageGestures(row, m) {
  ...
  // v3.22: tap on someone else's text message opens the Copy/Reply panel.
  row.addEventListener('click', function(ev) {
    ...
    // v3.22: dismiss any open click-panel on a fresh tap before deciding
    ...
    // v3.22: show Copy/Reply panel for OTHER people's text messages.
...
function leaveCall() {
  log("leave");
  leaving = true;
  // v3.22: belt-and-suspenders — fire the beacon AND close the WS. The
...
// v3.22: navigator.sendBeacon is the ONLY reliable way to send a payload
...
// v3.22: fire the beacon on every plausible unload signal. Different
...
// ═══════════════════════════════════════════════════════════════════════
// v3.23 GAMES + UNO CLIENT MODULE — self-contained
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// v3.27 HEADER DROPDOWN + STREAMING CLIENT
// ═══════════════════════════════════════════════════════════════════════
...
function toggleHeaderMenu(ev) {
  ...
  // v3.31: if the debug pane is currently visible, a single tap on the
...
const streamCli = {
  ...
  // serverId, error}
  queue: [],
  // v3.28: per-server-id mapping back to the local blob URL. The
  ...
  isStreamer: false,
  // v3.29: default to UNmuted. We try playing with sound first; the
...
function onStreamFilesPicked(ev) {
  ...
  for (const f of files) {
    ...
    const item = {
      ...
      size: f.size,
      file: f,
      // v3.28: blob: URL for instant local playback by the streamer.
  ...
  xhr.onload = () => {
    try {
      ...
      if (xhr.status >= 200 && xhr.status < 300 && j.id) {
        ...
        // v3.28: remember the blob URL so the streamer's local <video>
...
function removeStreamItem(tmpId) {
  // v3.28: revoke the blob URL so the browser can reclaim memory.
...
function streamHandleState(state) {
  if (!state) {
    ...
    // v3.28: revoke any blob URLs we held so the browser frees the
    ...
    // v3.34: restore the seat panel to its normal expanded state.
  ...
  // v3.34: when a stream first starts, auto-collapse the seat panel so
  ...
  // v3.29: swap the play-button icon between ▶ and ⏸ based on state.
  ...
  // v3.32: only show controls on specific transitions, not every state
...
function syncVideoToState(state, prevState) {
  ...
  // v3.28: streamer plays from their local blob URL whenever possible —
  ...
  // v3.29: try with sound on first. Users have almost always interacted
...
// ── v3.31: auto-hiding controls overlay ──
...
// v3.28: restored function (was eaten by an earlier refactor). Opens
...
function openUno() {
  ...
  // v3.26: every player gets the same fixed background image from
...
// ═══════════════════════════════════════════════════════════════════════
// v3.26 SHARED: Fixed game backgrounds + sound effects
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// v3.26 SOUND EFFECTS — procedural Web Audio (no asset files needed)
// ═══════════════════════════════════════════════════════════════════════
...
const sfx = {
  ...
  // v3.26: real card-flick sound. The old version was a square-wave
  ...
  // v3.26: much more zombie-y groan. The previous version was a single
...
// ════════════════════════════════════════════════════════════════════════════
// v4.0 OVERRIDE BLOCK — neutralize the old P2P-mesh WebRTC functions
// ════════════════════════════════════════════════════════════════════════════
// These functions used to drive the RTCPeerConnection mesh. With LiveKit
// owning the media plane, they have no work to do — but rather than rip
// every call site out of ~2000 lines of code, we just re-bind them here
// as no-ops. Anything that was previously calling createOffer(pid) etc.
// now harmlessly does nothing.
//
// IMPORTANT: this block must run AFTER the original function definitions
// — which is why we sit just before </script>.

(function neutralizeOldMesh() {
  function noop(){}
  function noopAsync(){ return Promise.resolve(); }

  try {
    // Peer connection lifecycle — replaced by LiveKit room.connect()
    if (typeof window.createOffer      !== 'undefined') window.createOffer      = noopAsync;
    if (typeof window.handleOffer      !== 'undefined') window.handleOffer      = noopAsync;
    if (typeof window.handleAnswer     !== 'undefined') window.handleAnswer     = noopAsync;
    if (typeof window.handleIce        !== 'undefined') window.handleIce        = noopAsync;
    if (typeof window.forceIceRestart  !== 'undefined') window.forceIceRestart  = noopAsync;
    if (typeof window.scheduleRetry    !== 'undefined') window.scheduleRetry    = noopAsync;
    if (typeof window.startConnectionTimer !== 'undefined') window.startConnectionTimer = noop;
    if (typeof window.clearConnectionTimer !== 'undefined') window.clearConnectionTimer = noop;
    if (typeof window.setupPC          !== 'undefined') window.setupPC          = noop;

    // ICE/TURN fetch — LiveKit ships its own ICE config in the token
    if (typeof window.fetchIceServers  !== 'undefined') {
      window.fetchIceServers = async function noopFetchIce() { return []; };
    }

    // Bitrate tuner — LiveKit's SFU manages bitrate per subscriber
    if (typeof window.applyBitrateToAll !== 'undefined') {
      window.applyBitrateToAll = noopAsync;
    }

    // SDP munging — meaningless without our own peer connections
    if (typeof window.preferOpusAndTune !== 'undefined') {
      window.preferOpusAndTune = function(sdp){ return sdp; };
    }

    // destroyPeer/nukePeer also tried to close RTCPeerConnections we no
    // longer have. We leave their other responsibilities (clearing audio
    // elements, timers) alone — the early-exit on `!peers[pid]` makes
    // them safe in the LK world.
  } catch (e) {
    try { log("v4.0 neutralize err: " + e.message); } catch(e2){}
  }

  // Hook leave/unload so we cleanly disconnect from LiveKit. The server
  // is also told via the existing `leave` WS message + beacon, but the
  // LK disconnect releases our publication immediately so other peers
  // see us drop without waiting for LK's keepalive timeout.
...
log("page loaded v4.0 (LiveKit voice, max " + MAX_PEERS + " peers)");
</script>
</body>
</html>"""
...
async def main():
    ...
    # v4.0: voice now flows through LiveKit. No more ICE/TURN warmup here.
    ...
    print("v3.22: rebuilt on v3.13 (kept simple reliable leave handling) + host avatar feature + Copy/Reply panel on message tap + sendBeacon leave for mobile reliability + explicit /leave WS message")
    print("v3.23: 🎮 GAMES + UNO — header game icon → games list → classic Uno (2/3/4 players); CSS-only cards; in-game side chat; voices keep working")
    print("v3.23.1: real SVG gamepad icon (no emoji), no chat auto-focus, winner overlay → Play Again + Close that fully resets, 30s turn timer with auto-draw/pass, card-fly animation on plays, haptic on your turn, smoother card press, layout-contain guard for hidden overlays")
    print("v3.23.2: +2/+4 STACKING — any color counters (Red+2 → Green+2 → Wild+4 → ... stack grows, last player without a counter takes the pile); max players raised 4 → 5; stack badge on discard pile + 'draw N or stack' turn hint")
    print("v3.24: 🧟 ZOMBIE GAME ADDED — Old-Maid variant for 2-5 players (matching pairs, last with the Zombie loses); Plato-style positional table layout for both games (opponents around screen edges with per-seat timer bars); winner overlay auto-dismissed when fresh game arrives so 'Play Again' works for everyone")
    print("v3.25: 🎨 host-only background picker (uno-bg/ + zombie-bg/ folders, image tile picker, broadcast to all); dark Plato-style default backgrounds; adaptive hand layout (10+ cards no longer overflow off-screen); recent-actions log under the discard pile; procedural sound effects (card play / draw / win / zombie groan / +stack pop); ZOMBIE RELAY recovery — peers stuck at dR=0 on relay for 20s now force-reset their PC instead of being marked 'track is LIVE'")
    print("v3.26: fixed default backgrounds — /uno-bg/bg1.jpg and /zombie-bg/bg1.jpg always used, no picker, no per-room state; in-game mute button in both Uno and Zombie headers (synced with the main call mute); paper-flick card play sound (was a click before); layered zombie groan with vibrato + sub-octave growl + vocal-cord rattle")
    print("v3.27: 📺 WATCH-TOGETHER STREAMING — 3-dot menu → Logs or Start Streaming; streamer uploads videos to a /stream-upload endpoint, builds a playlist, hits Start; server stores state + broadcasts; all viewers' <video> elements sync to the same currentTime / playing / track; seat panel shrinks while streaming so the video gets the prominent space; streamer-only controls (play, pause, seek, prev, next, stop); viewer 'tap to unmute' for mobile autoplay")
    print("v3.28: 🔧 streaming bugfixes — restored openGamesPicker() that an earlier refactor accidentally deleted (gamepad icon now opens picker again); simplified /stream-upload (single read instead of chunked loop) + diagnostic logs so stuck uploads can be debugged; streamer plays from local blob URL for instant playback (zero wait, no upload required for them to see their own video); viewers wait for the upload only; upload errors now surface real reasons instead of stuck spinners")
    print("v3.29: ✨ streaming polish — video shows at its natural aspect ratio (was being cropped by a forced 16:9 box); seat panel actually shrinks now (previous shrink CSS targeted classes that don't exist — fixed to use .seat .seat-av .seat-name); play/pause button properly swaps between ▶ and ⏸ icons; try unmuted autoplay first, only fall back to muted+prompt if the browser blocks; centered pulsing 'tap to unmute' pill (was a full-width banner before); LIVE dot pulse for viewers; bigger play button with glow; nicer progress-bar hover")
    print("v3.30: fixed-aspect 16:9 video frame (like Twitch/Discord) — portrait videos appear centered with black bars on the sides, landscape fills the frame, none get cropped, chat keeps its space; orange play button (warm not corporate-blue); smaller buttons; orange-amber progress bar to match")
    print("v3.31: 🎚 auto-hiding stream controls — controls overlay sits ON TOP of the video at the bottom and fades out 2.5s after the last tap, giving the video much more room; tap the video to show them again; never auto-hide while paused; smaller buttons; 3-dot menu now single-tap-dismisses the logs pane when it's already open (was forcing a 3-tap dance to close)")
    print("v3.32: controls only show on tap — no more flashing in/out every 4.5s from the heartbeat; videos loop when the playlist ends (Stop button or streamer-leave required to actually end the stream)")
    print("v3.33: video shows in full — moved the title from a top-of-video strip (which was covering the top of the actual video) into the bottom controls bar, so nothing visually intrudes on the video frame anymore; added explicit object-position:center + max-width/max-height:100% defensive sizing so videos always letterbox correctly and never get cropped")
    print("v3.34: seat panel auto-collapses when a stream starts so the video gets the full vertical real estate; pull-tab still works to drag it back down manually if you want to see everyone's avatars; restores to normal when the stream ends")
    print("v3.35: seat panel stays at FULL size during streaming instead of using a separate shrunken mode — when you pull it down mid-stream, you see all the avatars at their normal size overlaid on top of the video (it was already absolutely positioned). Two states (hidden / full) instead of three (hidden / shrunken / full)")
    print("v3.36: 🖼 actual screen framing — video sits inside a rounded container with a thin warm-orange border, drop shadow, and a bit of padding around it, separating it from the chat below. Reads as a 'screen mounted in the room' instead of a video laid awkwardly into the chat flow")
    print("v4.0: 🎙 SFU VOICE via LiveKit — replaces P2P mesh + TURN entirely; rooms scale cleanly to 15+ peers; Gulf/MENA latency fixed by LiveKit's edge network; all voice logic lives in call_system.py for clean future edits")
    print("=" * 60)
