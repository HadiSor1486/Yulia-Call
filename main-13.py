...
async function lkConnect() {
  ...
  // v4.0: neutralize the old mesh-WebRTC functions IMMEDIATELY, before any
  ...
  try {
    ...
    room.on(RE.TrackSubscribed, (track, publication, participant) => {
      ...
      // v4.0: mark this peer as connected so the seat-tile quality dot
    ...
    // v4.0: LiveKit emits ConnectionQualityChanged with a quality enum
    ...
    // v4.0: keep lastHeardAt fresh while audio is actually flowing so the
...
// (which is why we sit at the very end of the script).
// NOTE: do NOT write the literal closing-script token in comments here —
// the HTML parser would close our script tag and render the rest as text.
...
(function neutralizeOldMesh() {
  ...
  try {
    ...
    window.createOffer      = noopAsync;
    window.handleOffer      = noopAsync;
    window.handleAnswer     = noopAsync;
    window.handleIce        = noopAsync;
    window.forceIceRestart  = noopAsync;
    window.scheduleRetry    = noopAsync;
    window.startConnectionTimer = noop;
    window.clearConnectionTimer = noop;
    window.setupPC          = noop;
    ...
    // elements, timers) alone — the early-exit on missing `peers[pid]`
    // makes them safe in the LK world.
  } catch (e) {
