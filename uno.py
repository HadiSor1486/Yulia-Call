...
# Per-room asyncio Task that fires when the current turn times out. Keyed
# by room_id. Cancelled and replaced on every turn change. Stored separately
# from Game so the engine itself stays pure / synchronous.
...
TURN_TIMEOUT_SECONDS = 30 # auto-act when player takes too long
...
class Game:
    ...
    def __init__(self, room_id: str, host_pid: str, max_players: int) -> None:
        ...
        # Wall-clock when the current turn began. Refreshed on every turn
        # advance. Clients use this to draw a countdown bar.
    ...
    def play_card(self, pid: str, card_id: str,
        ...
        Stacking rule (per user request):
          When draw_stack > 0 (a +2 or +4 is hanging over the current
          player), they MAY counter with another +2 OR a +4 — colour
          doesn't matter for the counter. The penalty adds to the stack
          (2+2=4, 2+4=6, etc.) and is pushed onto the NEXT player. If
          they can't / don't want to counter, they call draw_card which
          takes the full stack and skips their turn.
        """
        if self.phase != "playing":
        ...
        # ── Stacking branch ──
        # A draw stack is pending. The ONLY legal plays are another +2 or
        # a +4. Any other card is rejected and the player should call draw.
        if self.draw_stack > 0:
            if card["value"] not in ("+2", "+4"):
                return {"ok": False,
                        "error": "Must counter with +2/+4 or draw"}
            # +4 still requires a chosen color (it's a wild).
            if card["value"] == "+4" and chosen_color not in COLORS:
                return {"ok": False, "error": "Choose a color for the +4"}
            # Commit the counter.
            hand.pop(idx)
            ...
            if card["value"] == "+4":
                ...
                # +2 keeps its own color as the active color.
                self.active_color = card["color"]
            # Grow the stack.
            add = 2 if card["value"] == "+2" else 4
            self.draw_stack += add

            events: List[dict] = []
            name = next(p["name"] for p in self.players if p["pid"] == pid)
            label = _card_label(card)
            color_word = (f" → {_color_word(chosen_color)}"
                          if card["value"] == "+4" else "")
            events.append({
                "kind": "play", "peer_id": pid,
                "text": f"{name} stacked {label}{color_word} (+{add}, total {self.draw_stack})",
            })
            # Same "almost-uno" hint as normal play.
            if len(hand) == 1 and pid not in self.uno_called:
                events.append({"kind": "almost_uno", "peer_id": pid,
                               "text": f"{name} has 1 card left!"})
            # Win check (rare but possible — last card was a +2/+4 stack).
            if len(hand) == 0:
                self.winner_pid = pid
                self.phase = "finished"
            ...
            # Pass the (now larger) stack to the next player.
            self._advance_turn(skip=False)
            events.append({"kind": "draw_pending",
                           "text": f"Stack is now +{self.draw_stack}"})
            return {"ok": True, "events": events}

        # ── Normal (non-stacking) branch ──
        ...
        if v == "skip":
            ...
            self._advance_turn(skip=False)
            events.append({"kind": "draw_pending",
                           "text": "Next player: draw 2 or stack a +2/+4"})
        elif v == "+4":
            self.draw_stack = 4
            self._advance_turn(skip=False)
            events.append({"kind": "draw_pending",
                           "text": "Next player: draw 4 or stack a +2/+4"})
        else:
            self._advance_turn(skip=False)
    ...
    def public_state(self) -> dict:
        ...
        return {
            ...
            # Wall-clock seconds (UTC) when this turn started + timeout.
            # Client renders a draining progress bar from this. Sending the
            # timeout too lets the client adapt without code changes.
...
async def _broadcast_state(room: dict, room_id: str, send: SendFn) -> None:
    g = games.get(room_id)
    if not g:
        # Game vanished — make sure any stale timer is cancelled too.
        _cancel_turn_timer(room_id)
        return
    state = g.public_state()
    await send(None, {"type": "uno_state", "state": state})
    # Keep the turn timer in sync with broadcasts. If we're not playing,
    # cancel any timer. If we ARE playing, the schedule helper only starts
    # a NEW task when the prior one fired or was cancelled — it always
    # cancels first then schedules anew, so it's safe to call here.
    if g.phase == "playing":
...
# ── Turn timer: forces a stuck player to draw + pass after 30s ──
# We schedule one asyncio task per (room, turn_started_at) tuple. If the
# turn changes naturally (a player acts), we cancel the old task and start
# a fresh one. If the task wakes up and finds the turn is still the same,
# it forces an action (draw + pass) and broadcasts.
...
async def _turn_timeout_runner(room_id: str, expected_turn_started_at: float,
                                room: dict, send: SendFn) -> None:
    try:
        await asyncio.sleep(TURN_TIMEOUT_SECONDS + 0.5)  # +0.5s grace
    except asyncio.CancelledError:
        return
    g = games.get(room_id)
    if not g or g.phase != "playing":
        return
    # If the turn moved on, our timer is stale.
    if g.turn_started_at != expected_turn_started_at:
        return
    pid = g.current_pid()
    name = next((p["name"] for p in g.players if p["pid"] == pid), "?")
    events: List[dict] = [{"kind": "timeout", "peer_id": pid,
                           "text": f"⏱ {name} timed out"}]
    affected_hand_pid: Optional[str] = None

    # Auto-act:
    #   - If there's a pending draw (+2/+4), force the player to take it
    #     (this also advances the turn).
    #   - Else if they haven't drawn this turn, force a draw + pass.
    #   - Else (drawn but didn't play), force a pass.
    if g.draw_stack > 0:
        res = g.draw_card(pid)
        events.extend(res.get("events", []))
        affected_hand_pid = pid
    elif not g.drawn_this_turn:
        res_d = g.draw_card(pid)
        events.extend(res_d.get("events", []))
        affected_hand_pid = pid
        # After the forced draw, even if it's playable we pass — the
        # timed-out player doesn't get a free turn.
        if g.phase == "playing" and g.current_pid() == pid:
            res_p = g.pass_turn(pid)
            events.extend(res_p.get("events", []))
    else:
        # Already drew, just couldn't decide — pass them.
        res_p = g.pass_turn(pid)
        events.extend(res_p.get("events", []))

    await _broadcast_state(room, room_id, send)
    if affected_hand_pid:
        await _send_private_hands(room, room_id, send,
                                   only_pid=affected_hand_pid)
    await _emit_events(room, room_id, send, events)
    _schedule_turn_timer(room_id, room, send)


def _schedule_turn_timer(room_id: str, room: dict, send: SendFn) -> None:
    """Start (or refresh) the turn-timeout task for this room. Safe to call
    multiple times — if the existing task is already running for the same
    turn_started_at, it's left alone. Otherwise the old one is cancelled
    and a fresh one is started. No-op if the game isn't playing."""
    g = games.get(room_id)
    if not g or g.phase != "playing":
        _cancel_turn_timer(room_id)
        return
    ...
    if existing is not None and not existing.done():
        # If the existing task is for the current turn, leave it. We stash
        # the turn_started_at on the task object so we can compare.
        if getattr(existing, "_uno_turn_at", None) == g.turn_started_at:
            return
        existing.cancel()
    try:
    ...
    # Tag with the turn it's watching so we can de-dupe later.
    task._uno_turn_at = g.turn_started_at  # type: ignore[attr-defined]
...
async def handle_ws(room_id: str, peer_id: str, peer_name: str,
    ...
    # uno_close: host force-closes. Any participant can close once the
    # game is finished (so any player can clear the table when it ends).
    if mt == "uno_close":
    ...
    # uno_play_again: when a game has ended, anyone in the game can open a
    # fresh lobby with the same player count. Starts a brand-new Game and
    # auto-joins the requester. Other previous players see the new lobby
    # state and can join/leave like normal. This is the cleanest way to
    # "reset" the page after a win without forcing everyone to re-pick.
    if mt == "uno_play_again":
        if g.phase != "finished":
            await send(peer_id, {"type": "uno_error",
                                  "text": "Can only restart after a game"})
            return
        # Was this peer actually in the previous game?
        was_in = any(p["pid"] == peer_id for p in g.players)
        if not was_in:
            await send(peer_id, {"type": "uno_error",
        ...
        await _emit_events(room, room_id, send, [{
            "kind": "lobby_open", "peer_id": peer_id,
            "text": f"{peer_name} started a new Uno lobby ({prev_max} players)"
        }])
        return
