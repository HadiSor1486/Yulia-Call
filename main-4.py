...
# v3.25: per-room background selections for each game. Maps:
#   room_id -> {"uno": "bg1.jpg" or "", "zombie": "bg2.jpg" or ""}
# Empty string = use the default solid dark background. Only the room
# HOST can change these via uno_bg_set / zomb_bg_set messages. Updates
# are broadcast to everyone in the room so all players see the same
# table at the same time.
game_backgrounds: Dict[str, Dict[str, str]] = {}
...
# ── v3.25: game background endpoints ─────────────────────────────────────
# Same pattern as avatars/stickers: list returns available filenames, the
# single-file endpoint serves the bytes with strict filename validation.
# Backgrounds for each game live in their own folder so they can't be
# accidentally mixed up. Add `uno-bg/bg1.jpg`, `bg2.jpg`, ... to the repo
# and they appear immediately in the host's background picker.

def _bg_list(directory: str) -> list:
    """Return sorted list of safe background filenames in directory."""
    if not os.path.isdir(directory):
        return []
    out = []
    for f in sorted(os.listdir(directory)):
        if not SAFE_BG_NAME.match(f):
            continue
        if os.path.splitext(f)[1].lower() not in BG_EXTS:
            continue
        out.append(f)
    return out
...
@app.get("/uno-bg")
async def uno_bgs_list():
    return JSONResponse({"backgrounds": _bg_list(UNO_BG_DIR)})
...
@app.get("/zombie-bg")
async def zombie_bgs_list():
    return JSONResponse({"backgrounds": _bg_list(ZOMBIE_BG_DIR)})
...
async def ws_endpoint(ws: WebSocket, room_id: str, t: str = Query(...)):
    ...
    # v3.25: sync the active game backgrounds (uno + zombie) to the new
    # peer so they walk into the same table everyone else sees.
    if room_id in game_backgrounds:
        bgs = game_backgrounds[room_id]
        if bgs.get("uno"):
            try:
                await ws.send_json({"type": "uno_bg", "bg": bgs["uno"]})
            except Exception:
                pass
        if bgs.get("zombie"):
            try:
                await ws.send_json({"type": "zomb_bg", "bg": bgs["zombie"]})
            except Exception:
                pass
    ...
    try:
        while True:
            ...
            # ── v3.25: host-only background change. Validates the filename
            # against the real disk listing (so peers can't pick arbitrary
            # paths), stores it per-room, and broadcasts so every peer
            # rerenders the same backdrop. Two parallel types — one per
            # game — so changing Uno's bg doesn't disturb Zombie and v.v.
            if mt in ("uno_bg_set", "zomb_bg_set"):
                if not is_host:
                    await ws.send_json({"type": "uno_error",
                                         "text": "Only the host can change the background"})
                    continue
                bg = str(msg.get("bg", "")).strip()[:80]
                # Empty string = revert to default. Otherwise verify the
                # filename actually exists in the right directory.
                if bg:
                    if not SAFE_BG_NAME.match(bg):
                        await ws.send_json({"type": "uno_error",
                                             "text": "Invalid background"})
                        continue
                    target_dir = UNO_BG_DIR if mt == "uno_bg_set" else ZOMBIE_BG_DIR
                    if not os.path.isfile(os.path.join(target_dir, bg)):
                        await ws.send_json({"type": "uno_error",
                                             "text": "Background file not found"})
                        continue
                game_backgrounds.setdefault(room_id, {"uno": "", "zombie": ""})
                key = "uno" if mt == "uno_bg_set" else "zombie"
                game_backgrounds[room_id][key] = bg
                # Broadcast to everyone. The message TYPE the clients
                # listen for is the same prefix as their game module.
                out_type = "uno_bg" if mt == "uno_bg_set" else "zomb_bg"
                broadcast = {"type": out_type, "bg": bg}
                for p in room["peers"].values():
                    try:
                        await p["ws"].send_json(broadcast)
                    except Exception:
                        pass
                continue
        ...
        if not room["peers"]:
            async def cleanup_later():
                ...
                if room_id in rooms and not rooms[room_id]["peers"]:
                    ...
                    # v3.25: drop the per-room game background selections
                    if room_id in game_backgrounds:
                        del game_backgrounds[room_id]
...
/* v3.25: tiny host-only 3-dot menu button. Hidden by default; the JS
   shows it only when MY_ID === room host. */
.uno-menu-btn{width:36px;height:36px;border-radius:50%;border:none;background:transparent;color:rgba(255,255,255,0.65);cursor:pointer;display:flex;align-items:center;justify-content:center;padding:0;margin-left:-4px}
.uno-menu-btn svg{width:22px;height:22px}
.uno-menu-btn:hover{color:#fff;background:rgba(255,255,255,0.06)}
.uno-menu-btn:active{transform:scale(0.92)}

/* Background picker — bottom-sheet that lists the available bg images */
.bg-picker-ovl{position:fixed;inset:0;z-index:265;background:rgba(0,0,0,0.7);backdrop-filter:blur(8px);display:none;align-items:flex-end;justify-content:center;animation:fadeIn .2s}
.bg-picker-ovl.show{display:flex}
.bg-picker-ovl:not(.show){contain:strict;pointer-events:none}
.bg-picker-box{width:100%;max-width:520px;background:linear-gradient(180deg,#1c1c1e,#0d0d0d);border-top-left-radius:20px;border-top-right-radius:20px;border-top:1px solid rgba(255,255,255,0.08);padding:12px 14px 22px;padding-bottom:calc(22px + env(safe-area-inset-bottom));max-height:80vh;overflow-y:auto;animation:slideUp .25s cubic-bezier(.2,.7,.2,1)}
.bg-picker-header{padding:6px 4px 14px}
.bg-picker-title{font-size:18px;font-weight:700;color:#fff;line-height:1.1}
.bg-picker-sub{font-size:11px;color:#888;margin-top:3px}
.bg-picker-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
.bg-picker-tile{position:relative;aspect-ratio:16/10;border-radius:12px;overflow:hidden;cursor:pointer;border:2px solid transparent;transition:border-color .15s,transform .12s;background:#1a1a1c}
.bg-picker-tile:active{transform:scale(0.97)}
.bg-picker-tile.sel{border-color:#ffd60a;box-shadow:0 0 16px rgba(255,214,10,0.35)}
.bg-picker-tile img{width:100%;height:100%;object-fit:cover;display:block}
.bg-picker-tile .bg-picker-label{position:absolute;inset:auto 0 0 0;padding:6px 8px;font-size:11px;color:#fff;font-weight:600;background:linear-gradient(180deg,transparent,rgba(0,0,0,0.75));text-shadow:0 1px 2px rgba(0,0,0,0.5)}
.bg-picker-tile.tile-default{background:linear-gradient(135deg,#1a1a1c,#0a0a0a);display:flex;align-items:center;justify-content:center;color:#888;font-weight:700;font-size:13px;letter-spacing:1px}
.bg-picker-empty{grid-column:1/-1;text-align:center;padding:30px 10px;color:#888;font-size:13px;line-height:1.5}
.bg-picker-empty code{background:rgba(255,255,255,0.06);padding:2px 6px;border-radius:5px;font-size:12px}
...
<div class="uno-ovl" id="unoOvl">
  <div class="uno-header">
    ...
    <button class="uno-menu-btn" id="unoHostMenuBtn" onclick="openBgPicker('uno')" title="Host menu" aria-label="Host menu" style="display:none">
...
<div class="zomb-ovl" id="zombOvl">
  <div class="uno-header">
    ...
    <button class="uno-menu-btn" id="zombHostMenuBtn" onclick="openBgPicker('zombie')" title="Host menu" aria-label="Host menu" style="display:none">
      <svg viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg>
    </button>
  </div>
...
<!-- v3.25: host-only background picker (shared between Uno and Zombie) -->
<div class="bg-picker-ovl" id="bgPickerOvl" onclick="if(event.target===this)closeBgPicker()">
  <div class="bg-picker-box">
    <div class="games-picker-handle"></div>
    <div class="bg-picker-header">
      <div class="bg-picker-title" id="bgPickerTitle">Background</div>
      <div class="bg-picker-sub">Visible to all players</div>
    </div>
    <div class="bg-picker-grid" id="bgPickerGrid">
      <!-- Filled dynamically. First tile is always "default" (dark). -->
    </div>
  </div>
</div>
...
function openUno() {
  ...
  // Show host-only menu button only for the room host.
  document.getElementById('unoHostMenuBtn').style.display = isHost ? '' : 'none';
  if (!unoCli.state) {
    unoShowLobbyCreate();
  } else {
    unoRender();
...
function openZombGame() {
  ...
  document.getElementById('zombHostMenuBtn').style.display = isHost ? '' : 'none';
  if (!zombCli.state) {
    zombShowLobbyCreate();
  } else {
    zombRender();
...
// ═══════════════════════════════════════════════════════════════════════
// v3.25 SHARED: Background picker (host-only) + sound effects
// ═══════════════════════════════════════════════════════════════════════
// The background picker is a small bottom-sheet that lists tiles for each
// image file the server reports in /uno-bg or /zombie-bg. Host taps a tile
// → we send {type: 'uno_bg_set' | 'zomb_bg_set', bg: filename}. Server
// validates host status, then broadcasts {type: 'uno_bg' | 'zomb_bg'} to
// every peer. Every client (including the host) applies the broadcast.
// This way we never trust the host's local state — the server is the
// single source of truth, exactly like the rest of the game.

const bgState = {
  currentGame: null,    // 'uno' or 'zombie' — which game the picker is showing
  selectedUno: '',      // currently-applied filename for Uno ('' = default)
  selectedZomb: '',     // currently-applied filename for Zombie
  cachedListUno: null,  // {fetched_at, items: [filename, ...]}
  cachedListZomb: null,
};

async function openBgPicker(game) {
  if (!isHost) return;  // belt-and-braces (button is also hidden)
  bgState.currentGame = game;
  document.getElementById('bgPickerTitle').textContent =
    (game === 'uno' ? 'Uno' : 'Zombie') + ' Background';
  const grid = document.getElementById('bgPickerGrid');
  grid.innerHTML = '<div class="bg-picker-empty">Loading...</div>';
  document.getElementById('bgPickerOvl').classList.add('show');
  // Fetch fresh list every time — directory contents may have changed
  // (someone added a new image to GitHub).
  const url = game === 'uno' ? '/uno-bg' : '/zombie-bg';
  let items = [];
  try {
    const r = await fetch(url, { cache: 'no-store' });
    if (r.ok) {
      const j = await r.json();
      items = Array.isArray(j.backgrounds) ? j.backgrounds : [];
    }
  } catch (e) {
    items = [];
  }
  if (game === 'uno') bgState.cachedListUno = items;
  else bgState.cachedListZomb = items;
  bgRenderTiles(game, items);
}

function closeBgPicker() {
  document.getElementById('bgPickerOvl').classList.remove('show');
  bgState.currentGame = null;
}

function bgRenderTiles(game, items) {
  const grid = document.getElementById('bgPickerGrid');
  grid.innerHTML = '';
  const current = (game === 'uno') ? bgState.selectedUno : bgState.selectedZomb;
  const urlBase = (game === 'uno') ? '/uno-bg/' : '/zombie-bg/';

  // Always show "default" tile first
  const def = document.createElement('div');
  def.className = 'bg-picker-tile tile-default' + (!current ? ' sel' : '');
  def.innerHTML = '<span>DEFAULT</span>';
  def.onclick = () => bgPickerSelect(game, '');
  grid.appendChild(def);

  if (!items.length) {
    const msg = document.createElement('div');
    msg.className = 'bg-picker-empty';
    const folder = (game === 'uno') ? 'uno-bg' : 'zombie-bg';
    msg.innerHTML = `Add image files to the <code>${folder}/</code> folder on GitHub<br>(jpg, png, webp · they'll appear here)`;
  ...
  items.forEach(filename => {
    ...
    tile.className = 'bg-picker-tile' + (current === filename ? ' sel' : '');
    tile.innerHTML =
      `<img src="${urlBase}${escHtml(filename)}" alt="" loading="lazy">
       <div class="bg-picker-label">${escHtml(filename)}</div>`;
    tile.onclick = () => bgPickerSelect(game, filename);
    grid.appendChild(tile);
  });
}

function bgPickerSelect(game, filename) {
  if (!ws || ws.readyState !== 1) return;
  const msgType = (game === 'uno') ? 'uno_bg_set' : 'zomb_bg_set';
  try {
    ws.send(JSON.stringify({ type: msgType, bg: filename }));
  } catch (e) {}
  closeBgPicker();
}

// Apply a background URL (or '' for default) to the Uno or Zombie overlay.
// Called both when our own host pick is echoed back AND when another host
// changed it before we joined (sync at connect time).
function applyGameBg(game, filename) {
  ...
  if (filename) {
    ...
    ovl.style.removeProperty((game === 'uno') ? '--uno-bg-image' : '--zomb-bg-image');
  }
  if (game === 'uno') bgState.selectedUno = filename || '';
  else bgState.selectedZomb = filename || '';
}

// ═══════════════════════════════════════════════════════════════════════
// v3.25 SOUND EFFECTS — procedural Web Audio (no asset files needed)
// ═══════════════════════════════════════════════════════════════════════
...
const sfx = {
  ...
  // Quick low-velocity click — used for card plays
  play() {
  ...
  // Zombie groan — detuned saws + low rumble, ~0.9s
  zombie() {
    if (!this.enabled) return;
    const ctx = this.ensureCtx();
    if (!ctx) return;
    const t = ctx.currentTime;
    // Two slightly detuned sawtooth waves at low frequency
    const make = (freq, detune, dur) => {
      const o = ctx.createOscillator();
      o.type = 'sawtooth';
      o.frequency.value = freq;
      o.detune.value = detune;
      // Slow pitch wobble — the "uuuugh" character
      o.frequency.setValueAtTime(freq, t);
      o.frequency.exponentialRampToValueAtTime(freq * 0.55, t + dur * 0.5);
      o.frequency.exponentialRampToValueAtTime(freq * 0.7, t + dur);
      const g = ctx.createGain();
      g.gain.setValueAtTime(0.0001, t);
      g.gain.exponentialRampToValueAtTime(0.12, t + 0.1);
      g.gain.setValueAtTime(0.12, t + dur - 0.18);
    ...
    make(110, 0, 0.95);
    make(110, -22, 0.95);
    // Plus a tiny "throat click" at the start
    const c = ctx.createOscillator();
    const cg = ctx.createGain();
    c.type = 'square'; c.frequency.value = 80;
...
// Receivers for bg broadcast — these are NOT prefixed with uno_/zomb_ in
// the routing logic above; we route them here directly because they're
// simple state messages. Add them to the existing routers by adding two
// extra case names. (Implemented as wrappers below so we don't have to
// edit the original switch blocks.)
const _origUnoHandleMsg = (typeof unoHandleServerMsg === 'function') ? unoHandleServerMsg : null;
unoHandleServerMsg = function(m) {
  if (m && m.type === 'uno_bg') {
    applyGameBg('uno', m.bg || '');
    return;
  }
  if (_origUnoHandleMsg) _origUnoHandleMsg(m);
};

const _origZombHandleMsg = (typeof zombHandleServerMsg === 'function') ? zombHandleServerMsg : null;
zombHandleServerMsg = function(m) {
  if (m && m.type === 'zomb_bg') {
    applyGameBg('zombie', m.bg || '');
    return;
  }
  if (_origZombHandleMsg) _origZombHandleMsg(m);
};
...
log("page loaded v3.22 (max " + MAX_PEERS + " peers, stickers, seat-grid, msg deletion, swipe reply, view once, host avatars, copy/reply, beacon leave)");
</script>
</body>
</html>"""
