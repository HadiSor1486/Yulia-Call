...
async def ws_endpoint(ws: WebSocket, room_id: str, t: str = Query(...)):
    ...
    # candidate's last_pong is OLDER than 30s — strong evidence the socket
    # is rotting (healthy peers pong every 15s). This guards against the
    # legitimate case of two real users sharing the same display name
    # (e.g. two "Leo"s in the room); they'll both have fresh last_pongs
...
function connectWS() {
  ...
  ws.onopen = () => {
    ...
    // network blip" on the local client side — old peer_ids get nuked here
    // and the new ones come back via the peers/peer_joined messages.
    if (MY_ID) {
...
log("page loaded v3.19 (max " + MAX_PEERS + " peers, stickers, seat-grid, msg deletion, swipe reply, view once, BULLETPROOF CALLING + WATCHDOG + WS heartbeat + zombie cleanup + HOST_AVATARS fix)");
</script>
</body>
</html>"""
