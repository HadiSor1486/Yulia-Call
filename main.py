...
/* v3.29: drop the forced 16:9 aspect — that was cropping/squashing videos
   whose natural aspect didn't match. Now the video element decides its
   own size from the source; the wrap just caps height so we always
   leave room for the seat strip + chat. */
.stream-video-wrap{position:relative;width:100%;background:#000;max-height:60vh;display:flex;align-items:center;justify-content:center;overflow:hidden}
.stream-video-wrap video{width:100%;height:auto;max-height:60vh;object-fit:contain;background:#000;display:block}
...
.stream-controls{padding:10px 14px 12px;background:linear-gradient(180deg,#0a0a0a,#000);color:#fff}
.stream-title{font-size:13px;font-weight:600;opacity:0.92;margin-bottom:8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;letter-spacing:0.1px}
.stream-progress{position:relative;height:5px;background:rgba(255,255,255,0.12);border-radius:3px;cursor:pointer;touch-action:none;transition:height .12s}
body.streaming.is-streamer .stream-progress{cursor:pointer}
body.streaming.is-streamer .stream-progress:hover{height:7px}
.stream-progress-fill{position:absolute;left:0;top:0;bottom:0;background:linear-gradient(90deg,#1a7ad8,#5eaaff);border-radius:3px;width:0%;transition:width .15s linear}
.stream-progress-handle{position:absolute;top:50%;transform:translate(-50%,-50%);width:13px;height:13px;border-radius:50%;background:#fff;left:0%;display:none;box-shadow:0 1px 4px rgba(0,0,0,0.6),0 0 0 2px rgba(26,122,216,0.4)}
...
.stream-buttons{display:flex;align-items:center;justify-content:center;gap:18px;padding:10px 0 2px}
/* Viewers see only the play indicator (read-only display) */
body.streaming:not(.is-streamer) .stream-buttons{display:none}
body.streaming.is-streamer .stream-viewer-info{display:none}
.stream-viewer-info{text-align:center;font-size:12px;color:#aaa;padding:6px 0 2px;font-weight:500}
...
.stream-btn{width:42px;height:42px;border-radius:50%;border:none;background:rgba(255,255,255,0.1);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;padding:0;transition:transform .1s,background .15s}
.stream-btn:hover{background:rgba(255,255,255,0.16)}
...
.stream-btn-play{width:54px;height:54px;background:#1a7ad8;box-shadow:0 4px 16px rgba(26,122,216,0.35)}
.stream-btn-play:hover{background:#2a8ae8}
.stream-btn-play svg{width:26px;height:26px}
.stream-btn-stop{background:rgba(255,59,48,0.18);color:#ff5f5a}
.stream-btn-stop:hover{background:rgba(255,59,48,0.32)}
.stream-btn-stop svg{width:16px;height:16px}
