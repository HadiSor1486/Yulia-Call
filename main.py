...
async def ws_endpoint(ws: WebSocket, room_id: str, t: str = Query(...)):
    ...
    # v3.18: ZOMBIE EXPULSION. Look for stale connections with the same name
    # that are likely dead but haven't been cleaned up yet (mobile network
    # drops can take 30-90s to surface as TCP closes). We only expel if the
    # candidate's last_pong is OLDER than 30s — that's strong evidence the
    # socket is rotting (healthy peers pong every 15s). This guards against
    # the legitimate case of two real users sharing the same display name
    # (e.g. two "Leo"s in the room); they'll both have fresh last_pongs and
    # we won't touch them.
    zombies = []
    now_ts = time.time()
    for existing_pid, existing_pd in list(room["peers"].items()):
        if existing_pd["name"] != name:
            continue
        last_p = existing_pd.get("last_pong", existing_pd.get("joined", 0))
        if now_ts - last_p > 30:
            zombies.append(existing_pid)
            print(f"[WS] zombie candidate {existing_pid} ({name}) last_pong={now_ts - last_p:.1f}s ago")
    for zpid in zombies:
        zpd = room["peers"].get(zpid)
        if zpd:
            print(f"[WS] expelling zombie {zpid} ({name}) — same name rejoined as new connection")
            # Mark BEFORE closing so when the zombie's finally block runs,
            # it sees the marker and skips the duplicate broadcast.
            expelled_peers.add(zpid)
            try:
                # Send a hint to anyone else that this peer is leaving so their
                # client clears the seat immediately, BEFORE the new join lands.
                # This avoids the brief flicker where both old + new tiles exist.
                lm = {"type": "peer_left", "peer_id": zpid, "name": name}
                for op, opd in room["peers"].items():
                    ...
                    try:
                        ...
                        pass
                # Close the zombie's socket. Its disconnect handler will run
                # (no-op the broadcast since we already sent peer_left).
                await zpd["ws"].close(code=4005)
            except Exception:
                pass
            # Remove from peer list immediately so the new join sees a clean
            # slate. The zombie's finally block will see peer_id no longer
            # in room["peers"] and skip the duplicate broadcast.
            if zpid in room["peers"]:
                del room["peers"][zpid]
    ...
    room["peers"][peer_id] = {
        ...
        "joined": time.time(),
        # v3.18: track liveness so the zombie-expulsion logic can tell apart
        # an actual zombie (stale last_pong) from a healthy duplicate-name peer.
        "last_pong": time.time(),
    }
    ...
    ping_task = None
    # v3.18: server-side liveness tracking. Pongs reset last_pong. If we go
    # >50s without a pong, the socket is dead even if TCP hasn't noticed yet
    # (common on mobile networks where the connection silently rots). We
    # force-close to trigger the disconnect handler and broadcast peer_left
    # quickly instead of waiting for the next receive_json to fail.
    last_pong = [time.time()]

    async def pinger():
        try:
            while True:
                await asyncio.sleep(15)  # was 20s; faster detection
                try:
                    await ws.send_json({"type": "ping", "t": time.time()})
                except Exception:
                    return
                # If we haven't received a pong in 50s, kill the socket.
                # 50s = comfortable margin: 3 missed pings (15s each = 45s)
                # + small buffer for network jitter.
                if time.time() - last_pong[0] > 50:
                    print(f"[WS] {peer_id} pong timeout — forcing disconnect")
                    try:
...
function connectWS() {
  ...
  ws.onopen = () => {
    log("WS open");
    // v3.18: detect reconnect (we had a session before). Clean up all
    // existing peer state — the WebRTC connections went through a signaling
    // blackout and any in-flight ICE candidates / restart offers are stale.
    // The fresh peer_list message will rebuild from scratch with correct ids.
    // This is the cure for "zombie tiles stuck on screen after my friend's
    // network blip" — old peer_ids get nuked here and the new ones come
    // back via the peers/peer_joined messages.
    if (MY_ID) {
      log("WS reconnect detected — cleaning " + peerMap.size + " stale peer(s)");
      for (const [pid, _] of peerMap) {
        try { nukePeer(pid); } catch (e) {}
      }
      peerMap.clear();
      // Clear any in-flight reconnect bookkeeping so nothing fires against
      // a destroyed peer.
      Object.keys(_failedWatchdogs).forEach(function(pid) {
        clearTimeout(_failedWatchdogs[pid]);
        delete _failedWatchdogs[pid];
      });
      // Reset MY_ID so we know to expect a fresh your_id from the server.
      MY_ID = null;
      updPeers();
      updCount();
    }
  ...
  ws.onclose = e => {
    ...
    // v3.18: 4001 = token/room invalid. Server has cleaned up the room (probably
    // after the 5min empty grace) or our token is rejected. Looping forever
    // will never succeed. Show a clear UI message and stop the retry storm.
    if (e.code === 4001) {
    ...
    // 4005 = server-side zombie expulsion (we joined again from another tab/device).
    // Don't retry — the new session is alive elsewhere.
    if (e.code === 4005) {
...
log("page loaded v3.18 (max " + MAX_PEERS + " peers, stickers, seat-grid, msg deletion, swipe reply, view once, BULLETPROOF CALLING + WATCHDOG + WS heartbeat + zombie-cleanup)");
</script>
</body>
</html>"""
