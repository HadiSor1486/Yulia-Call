...
.uno-ovl{position:fixed;inset:0;z-index:230;background:radial-gradient(ellipse at center,#1a3d1a 0%,#0a1a0a 70%,#000 100%);display:none;flex-direction:column;animation:fadeIn .2s}
.uno-ovl.show{display:flex}

/* Felt-table noise overlay for that real card-table feel */
.uno-ovl::before{content:'';position:absolute;inset:0;background-image:radial-gradient(circle at 25% 30%, rgba(255,255,255,0.02) 0%, transparent 40%),radial-gradient(circle at 75% 70%, rgba(255,255,255,0.02) 0%, transparent 40%);pointer-events:none}

.uno-header{position:relative;z-index:2;padding:8px 12px;display:flex;align-items:center;gap:10px;background:rgba(0,0,0,0.4);border-bottom:1px solid rgba(255,255,255,0.06);flex-shrink:0}
...
.uno-body{position:relative;z-index:1;flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden}
...
.uno-player-count-row{display:flex;gap:8px}
.uno-player-count-btn{flex:1;padding:14px 0;border-radius:12px;border:2px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04);color:#fff;font-weight:700;font-size:16px;cursor:pointer;transition:all .15s}
...
.uno-play{flex:1;display:flex;flex-direction:column;min-height:0}
.uno-opponents{display:flex;justify-content:center;gap:10px;padding:12px 10px 4px;flex-wrap:wrap;flex-shrink:0}
.uno-opp{display:flex;flex-direction:column;align-items:center;gap:4px;min-width:60px;max-width:90px;opacity:0.6;transition:opacity .2s,transform .2s}
.uno-opp.turn{opacity:1;transform:scale(1.05)}
.uno-opp-av-wrap{position:relative;width:48px;height:48px}
.uno-opp-av{width:48px;height:48px;border-radius:50%;object-fit:cover;background:#333;display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff;font-size:18px;border:2px solid rgba(255,255,255,0.1)}
.uno-opp.turn .uno-opp-av{border-color:#ffd60a;box-shadow:0 0 14px rgba(255,214,10,0.6)}
.uno-opp-back-pile{position:absolute;top:-4px;right:-12px;display:flex}
.uno-opp-mini{width:18px;height:26px;border-radius:3px;background:linear-gradient(135deg,#222,#000);border:1px solid #444;margin-left:-12px;position:relative;box-shadow:0 1px 2px rgba(0,0,0,0.5)}
.uno-opp-mini::after{content:'U';position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#e10600;font-style:italic;font-weight:900;font-size:10px;text-shadow:0 1px 0 #ffd60a}
.uno-opp-name{font-size:11px;color:#fff;font-weight:500;max-width:80px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center}
.uno-opp-count{font-size:10px;color:#ffd60a;font-weight:700}
.uno-opp.uno-flag .uno-opp-count{color:#fff;background:#e10600;padding:1px 6px;border-radius:8px;animation:unoFlash 1s ease-in-out infinite}

@keyframes unoFlash{0%,100%{transform:scale(1)}50%{transform:scale(1.15)}}

/* Table center — discard + draw pile */
.uno-table{flex:1;display:flex;align-items:center;justify-content:center;gap:22px;padding:8px;min-height:0}
.uno-pile{position:relative;display:flex;flex-direction:column;align-items:center;gap:6px}
.uno-pile-label{font-size:10px;color:#fff;opacity:0.6;font-weight:700;letter-spacing:1px;text-transform:uppercase}
...
.uno-turn-bar{padding:6px 14px;text-align:center;font-size:13px;font-weight:600;color:#fff;background:rgba(0,0,0,0.4);flex-shrink:0}
.uno-turn-bar.your-turn{background:linear-gradient(90deg,rgba(255,214,10,0.25),rgba(255,214,10,0.05),rgba(255,214,10,0.25));color:#ffd60a;animation:turnPulse 2s ease-in-out infinite}
...
/* Turn timer bar — drains over TURN_TIMEOUT_SECONDS (30s by default).
   Fill colors green→yellow→red as time runs out. Below 5 seconds it
   pulses to warn the active player. */
.uno-turn-timer{position:relative;height:3px;background:rgba(255,255,255,0.06);overflow:hidden;flex-shrink:0}
.uno-turn-timer-fill{position:absolute;left:0;top:0;bottom:0;width:0%;background:linear-gradient(90deg,#0d8a3d,#ffd60a 70%,#e10600);transition:width .25s linear;will-change:width}
.uno-turn-timer-fill.warn{animation:timerPulse .6s ease-in-out infinite}
...
.uno-hand .uno-card{margin-left:-26px;transition:transform .18s cubic-bezier(.2,.7,.2,1.2),margin-left .2s}
    ...
    <div class="games-picker-list">
      <div class="games-picker-item" onclick="onPickGameUno()">
        ...
        <div class="games-picker-info">
          ...
          <div class="games-picker-desc">2-4 players · Match colors and numbers</div>
        </div>
      </div>
...
<div class="uno-ovl" id="unoOvl">
  ...
  <div class="uno-body" id="unoBody">
    ...
    <div class="uno-play" id="unoPlay" style="display:none">
      ...
      <div class="uno-turn-timer" id="unoTurnTimer"><div class="uno-turn-timer-fill" id="unoTurnTimerFill"></div></div>
      <div class="uno-table">
        <div class="uno-pile">
        ...
        <div class="uno-pile">
          <div class="uno-pile-label">DISCARD <span class="uno-color-dot" id="unoColorDot"></span></div>
...
function openUno() {
  ...
  // If there's an active server-side state for this room (someone else
  // already started a lobby/game), we'll get a fresh broadcast soon. Otherwise
  // render the lobby create view.
  if (!unoCli.state) {
    unoShowLobbyCreate();
  } else {
    unoRender();
  }
  // Seed the chat panel with the recent mirror buffer (so people who open
  // the panel mid-conversation see context).
  unoRenderChat();
}
function onUnoBack() {
...
function unoRenderPlay() {
  ...
  // Opponents row (everyone except me)
  const oppEl = document.getElementById('unoOpponents');
  oppEl.innerHTML = '';
  s.players.forEach(p => {
    if (p.pid === MY_ID) return;
    ...
    div.className = 'uno-opp' + (isTurn ? ' turn' : '') + (count === 1 ? ' uno-flag' : '');
    const initial = escHtml((p.name || '?').slice(0, 1).toUpperCase());
    const av = p.avatar
      ? `<img class="uno-opp-av" src="${escHtml(p.avatar)}" onerror="this.style.display='none'">`
      : `<div class="uno-opp-av">${initial}</div>`;
    div.innerHTML =
      `<div class="uno-opp-av-wrap">${av}<div class="uno-opp-back-pile">${'<div class="uno-opp-mini"></div>'.repeat(Math.min(count, 3))}</div></div>
  ...
  if (s.winner) {
    ...
    if (s.draw_pending > 0) {
      turnBar.textContent = `Your turn — draw ${s.draw_pending}!`;
    } else if (s.must_pass) {
...
// Drives the turn-timer bar. requestAnimationFrame loop that reads
// turn_started_at + turn_timeout from server state, computes elapsed
// fraction, and updates the fill width. Self-stops when the game isn't
// playing or the overlay is closed. We compute server-time using a
// one-shot delta captured from the first state's server_now field, which
// keeps clients in sync even if their device clocks are off.
...
function unoStartTimerLoop() {
  ...
  const tick = () => {
    unoTimerRaf = null;
    const s = unoCli.state;
    const bar = document.getElementById('unoTurnTimer');
    const fill = document.getElementById('unoTurnTimerFill');
    if (!bar || !fill) return;
    ...
    // Clock-skew correction. The server includes server_now in every
    // broadcast; we capture the offset from the first sample and reuse
    // it. Don't re-derive on every state — that would lose monotonicity.
    ...
    // We don't want CSS transitions fighting with RAF — disable transition
    // and just set width directly. RAF gives us ~60fps, smoother than the
    // 250ms CSS transition we had.
    fill.style.transition = 'none';
    fill.style.width = pct + '%';
    fill.classList.toggle('warn', remaining < 5 && remaining > 0);
...
function unoCardIsPlayable(card) {
  ...
  if (unoCli.state.draw_pending > 0) return false;  // must draw first
