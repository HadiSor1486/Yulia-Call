...
async def ws_endpoint(ws: WebSocket, room_id: str, t: str = Query(...)):
    ...
    # v3.20: ACTIVE LIVENESS PROBE. The static last_pong threshold (>30s)
    # had a hole: if a user closed their tab and rejoined within 30 seconds,
    # the old WS still showed a "fresh" pong timestamp from just before
    # death. The probe avoids this entirely: when a same-name peer exists,
    # we send them a live ping, mark our expectation, and wait up to 2s for
    # the pong. If no pong arrives (because the socket is actually dead),
    # we expel. If a pong DOES arrive, the old session is genuinely alive
    # and we treat it as a duplicate-name case (two real users named the
    # same thing) and let both coexist.
    zombies = []
    same_name_candidates = []
    for existing_pid, existing_pd in list(room["peers"].items()):
        if existing_pd["name"] == name:
            same_name_candidates.append((existing_pid, existing_pd))

    if same_name_candidates:
        print(f"[WS] join: {len(same_name_candidates)} existing peer(s) with name '{name}' — probing liveness")
        # Send probe pings to all same-name candidates simultaneously
        probe_results = {}
        for cpid, cpd in same_name_candidates:
            cpd["_probe_pending"] = True
            cpd["_probe_id"] = f"probe_{int(time.time()*1000)}_{cpid}"
            try:
                await cpd["ws"].send_json({"type": "ping", "t": time.time(), "probe": cpd["_probe_id"]})
                probe_results[cpid] = "sent"
            except Exception as e:
                # Send failed → socket already dead → expel immediately
                probe_results[cpid] = "send_failed"
                zombies.append(cpid)
                print(f"[WS] probe send failed for {cpid} ({name}) — {e!r} — expelling")

        # Wait up to 2 seconds for pongs to update last_pong
        if any(r == "sent" for r in probe_results.values()):
            probe_start = time.time()
            await asyncio.sleep(2.0)
            for cpid, cpd in same_name_candidates:
                if probe_results.get(cpid) != "sent":
                    continue
                if cpid not in room["peers"]:
                    continue  # already gone (its loop ended during our sleep)
                last_p = room["peers"][cpid].get("last_pong", 0)
                if last_p < probe_start:
                    # No pong since we sent the probe → dead socket
                    zombies.append(cpid)
                    print(f"[WS] probe timeout for {cpid} ({name}) — no pong in 2s — expelling")
                else:
                    print(f"[WS] probe OK for {cpid} ({name}) — keeping alive")
                    room["peers"][cpid].pop("_probe_pending", None)
                    room["peers"][cpid].pop("_probe_id", None)

    for zpid in zombies:
        zpd = room["peers"].get(zpid)
        if not zpd:
            continue
        print(f"[WS] expelling zombie {zpid} ({name})")
        expelled_peers.add(zpid)
        try:
            # v3.20: also send the "X left the call" system chat so users see
            # what happened. We dedupe via the expelled flag so the leaving
            # peer's finally block won't re-broadcast. Without this, the seat
            # cleared silently and looked buggy.
...
function showAvatarPicker(pid, anchorEl) {
  ...
  HOST_AVATARS.forEach(function(url, idx) {
    ...
    item.addEventListener('click', function(ev) {
      ev.stopPropagation();
      clickLog('avatar-pick', { pid: pid, name: p.name, avatarUrl: url, index: idx });
  ...
  dlog('AV-PICKER', 'picker appended to DOM at ' + left + ',' + top);
}

function _onAvPickerDismiss(e) {
  if (_avPickerEl && !_avPickerEl.contains(e.target)) {
    dlog('AV-PICKER', 'dismissed by outside click');
    hideAvatarPicker();
  }
}
...
// to assign an avatar. THIS is why clicking did nothing.
const HOST_AVATARS = [
...
log("page loaded v3.20 (max " + MAX_PEERS + " peers, stickers, seat-grid, msg deletion, swipe reply, view once, BULLETPROOF CALLING + WATCHDOG + WS heartbeat + active liveness probe + reconnect leave notifications + click debug)");
</script>
</body>
</html>"""
