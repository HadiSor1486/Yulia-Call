...
async def stream_upload(request: Request, room_id: str = Query(...),
    ...
    # Streaming policy: only one stream may be active per room. If a
    # different peer already started, reject the upload.
    existing = streams.get(room_id)
    # We CAN allow uploads before stream_start (to build up the playlist
    # first). The existence check uses streamer_pid to detect "someone
    # else is already streaming". The token-bound peer who's uploading is
    # whoever holds the token — which is also their WS peer_id binding.
    # We don't have the peer_id directly here, but we can look it up via
    # the token (tokens map to {room_id, peer_id, ...} if you track that;
    # if you don't, simply check the streamer_pid matches the token's owner).
    # Simpler: if a stream is already going under SOMEONE, reject; the
    # streamer must stop first.
    if existing and existing.get("playing") is not None:
        # If files have already been linked to a playlist, this room is
        # streaming. We allow the same streamer to add more files via
        # the WS message instead.
        if existing.get("playlist"):
            return JSONResponse({"error": "A stream is already running"}, 409)

    # File size guard — we stream the body so we can stop early if it's
    # too big rather than buffering the whole thing in RAM.
    form = await request.form()
    upload = form.get("file")
    if upload is None:
        return JSONResponse({"error": "No file"}, 400)

    # The original filename is just for display/title — never used as a
    # disk path. Strip directories defensively.
    ...
    # Read into a random file. We chunk so we can abort on size cap.
        ...
        with open(out_path, "wb") as fh:
            while True:
                ...
                if not chunk:
                    break
                total += len(chunk)
                if total > MAX_STREAM_FILE_BYTES:
                    fh.close()
                    _stream_cleanup_paths([out_path])
    ...
    # Strip extension from display title; cap length.
...
/* When a stream is active, body gets the .streaming class. The seat
   ...
   to the shared video. */
body.streaming .seat-panel{max-height:78px}
body.streaming .seat-tile{padding:4px 0 2px}
body.streaming .seat-tile-av-wrap{width:42px;height:42px}
body.streaming .seat-tile-name{font-size:10px;margin-top:1px}
body.streaming .seat-handle{display:none}
/* Speaking ring is recreated smaller too. */
body.streaming .seat-tile-av{width:42px;height:42px;font-size:16px}
...
.stream-video-wrap{position:relative;width:100%;background:#000;aspect-ratio:16/9;max-height:48vh;overflow:hidden}
.stream-video-wrap video{width:100%;height:100%;object-fit:contain;background:#000;display:block}
.stream-tap-shield{position:absolute;inset:0;cursor:pointer;z-index:2}
.stream-tap-shield.streamer-mode{cursor:default}
.stream-unmute-prompt{position:absolute;inset:auto 0 0 0;display:none;align-items:center;justify-content:center;gap:8px;padding:10px;background:rgba(0,0,0,0.7);color:#fff;font-size:13px;font-weight:600;cursor:pointer;z-index:3}
.stream-unmute-prompt.show{display:flex}
.stream-unmute-prompt svg{width:18px;height:18px}

.stream-controls{padding:8px 12px 10px;background:linear-gradient(180deg,#0a0a0a,#000);color:#fff}
.stream-title{font-size:13px;font-weight:600;opacity:0.95;margin-bottom:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.stream-progress{position:relative;height:6px;background:rgba(255,255,255,0.12);border-radius:3px;cursor:pointer;touch-action:none}
...
.stream-progress-handle{position:absolute;top:50%;transform:translate(-50%,-50%);width:14px;height:14px;border-radius:50%;background:#fff;left:0%;display:none;box-shadow:0 1px 4px rgba(0,0,0,0.5)}
body.streaming.is-streamer .stream-progress-handle{display:block}
.stream-time-row{display:flex;justify-content:space-between;font-size:10px;color:#aaa;margin-top:3px;font-variant-numeric:tabular-nums}
.stream-buttons{display:flex;align-items:center;justify-content:center;gap:14px;padding:6px 0 0}
...
.stream-viewer-info{text-align:center;font-size:11px;color:#aaa;padding:4px 0 0}
.stream-btn{width:38px;height:38px;border-radius:50%;border:none;background:rgba(255,255,255,0.08);color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;padding:0;transition:transform .1s,background .15s}
...
.stream-btn-play{width:46px;height:46px;background:#1a7ad8}
.stream-btn-play:hover{background:#2a8ae8}
.stream-btn-play svg{width:22px;height:22px}
.stream-btn-stop{background:rgba(255,59,48,0.2);color:#ff5f5a}
.stream-btn-stop:hover{background:rgba(255,59,48,0.35)}
...
const streamCli = {
  ...
  // "Start streaming". Each item: {tmpId, name, size, status:'pending'|
  // 'uploading'|'done'|'error', progress:0-1, serverId, error}
  queue: [],
  // Are we the streamer? Mirrors body.is-streamer.
  isStreamer: false,
  // Local "muted" flag for the video element. Autoplay-with-sound is
  // blocked on mobile, so we start muted and offer a "tap to unmute"
  // overlay. Once the user taps anywhere, we unmute.
  videoMuted: true,
...
function syncVideoToState(state, prevState) {
  ...
  // If src or idx changed, swap source.
  const newSrc = cur.url;
  const idxChanged = !prevState || prevState.idx !== state.idx;
  if (idxChanged || !v.currentSrc.endsWith(newSrc)) {
    streamCli.lastSeekedTo = -1;
    try {
      v.src = newSrc;
      // Most mobile browsers won't let us autoplay with sound on first
      // touch — start muted, then prompt the user to unmute.
