...
/* v3.33: bulletproof "always show the whole video" sizing.
   - `aspect-ratio:16/9` gives the wrap a fixed landscape shape so chat
     keeps its space below.
   - The video inside is sized with explicit width/height + object-fit:
     contain + object-position:center, so EVERY part of the video frame
     fits inside the wrap with letterbox bars where needed.
   - No max-height on the video element (a stray max-height was what
     caused earlier cropping on tall sources). */
.stream-video-wrap{position:relative;width:100%;aspect-ratio:16/9;background:#000;overflow:hidden;line-height:0}
